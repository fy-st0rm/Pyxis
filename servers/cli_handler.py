...
class Cli_Handler:
	...
	def signup(self, username, password):
		if username in self.cli_data: return pQuery(SERVER, UNKNOWN, FAILED, [f"Username `{username}` already exists. Please choose another one."], None, uuid.uuid4())
		...
		return pQuery(SERVER, UNKNOWN, SUCESS, [f"Sucessfully created a new account.", user_uuid, hashed], None, uuid.uuid4())

	def login(self, username, password):
		if username not in self.cli_data: return pQuery(SERVER, UNKNOWN, FAILED, [f"User with username `{username}` doesn`t exists."], None, uuid.uuid4())
		...
		if supplied_hashed != hashed: return pQuery(SERVER, UNKNOWN, FAILED, [f"Password for user `{username}` didn`t matched."], None, uuid.uuid4())

		return pQuery(SERVER, UNKNOWN, SUCESS, [f"Sucessfully logged in.", user_uuid, hashed], None, uuid.uuid4())
	
	# Query Handler
	def parse_query(self, qry):
		if   qry.cmd == SIGNUP: return self.signup(qry.params[0], qry.params[1])
		elif qry.cmd == LOGIN : return self.login(qry.params[0], qry.params[1])
